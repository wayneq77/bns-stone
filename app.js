...
// 2. 由於實機驗證，從「出生」到「下次出生」的完整大循環是 **140 分鐘** (DEFAULT_INTERVAL)。
//    => 所以靈石自然消失後，畫面的倒數計時必定會從 **120 分鐘** 開始倒數！
//       (20分存在 + 120分等待 = 140 分鐘)
// 
// [已撿完 (Collected) 狀態]
// - 當靈石出現期間，玩家按下「已撿完」。
// - 下一次出生時間會被提早 20 分鐘，大循環變成總共 120 分鐘。
// 
// [時間推演範例]
// 假設某靈石固定在 11:14 出生：
// 
// ▶ 情境 A (自然消失)：
//   - 11:14：靈石出生 (倒數 20 分鐘準備消失)
//   - 11:34：靈石消失 (進入等待期)
//   - 此時等待期倒數：因為總循環是 140 分鐘，所以這時畫面上會顯示 02:00:00 (120分鐘倒數)
//   - 13:34：靈石下次出生。 (11:14 + 140m = 13:34)  <-- 完全吻合遊戲內實際提示！
// ==============================================================================
...
const CONFIG = {
    // Default spawn interval: 140 min
