...
// ==============================================================================
// 靈石重生時間核心邏輯 (Core Spawn Logic)
// ==============================================================================
// [基本參數]
// 1. 靈石出現後，會存在 20 分鐘 (DESPAWN_WINDOW)。
// 2. 靈石自然消失後，需要等待 140 分鐘才會再次出現。
//    => 所以從「本次出生」到「下次出生」的完整大循環是：
//       20分(存在) + 140分(等待) = 160 分鐘 (DEFAULT_INTERVAL)。
// 
// [已撿完 (Collected) 狀態]
// - 當靈石出現期間，玩家按下「已撿完」。
// - 下一次出生時間會被提早 20 分鐘。
// - 也就是說，如果 1:00 消失，本來是 3:20 再出；如果是 1:00 撿完，就會提早在 3:00 再次出現！
// - 數學上：從「本次出生」到「下次出生」，若含有已撿完的加成，該循環為：
//   160 分鐘(原) - 20 分鐘(縮減) = 140 分鐘 (COLLECTED_INTERVAL)。
// 
// [時間推演範例]
// 假設某靈石固定在 12:40 出生：
// 
// ▶ 情境 A (自然消失)：
//   - 12:40：靈石出生 (倒數 20 分鐘準備消失)
//   - 13:00：靈石消失 (進入等待期)
//   - 此時等待期倒數：140 分鐘 （畫面上會顯示 02:20:00）
//   - 15:20：靈石下次出生。 (12:40 + 160m = 15:20)
// 
// ▶ 情境 B (按下已撿完)：
//   - 12:40：靈石出生
//   - 13:00：你在此刻按下「已撿完」 (或者出現時的任意時刻按)
//   - 系統計算下次出生為 12:40 + 140m = 15:00
//   - 此時距離 15:00 剩 120 分鐘 (畫面上顯示 02:00:00)
// ==============================================================================
